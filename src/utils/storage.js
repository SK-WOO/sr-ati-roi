...
export function saveHwPresets(list) {
  try { localStorage.setItem(HW_PRESET_KEY, JSON.stringify(list)); return true; }
...
export function saveCalcCache(data) {
  try { localStorage.setItem(CALC_CACHE_KEY, JSON.stringify(data)); return true; }
  catch (e) { return e.name === "QuotaExceededError" ? "quota" : false; }
}
